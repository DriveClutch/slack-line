...
app.post('/api/exchangeCode', function (req, res) {
    request('https://slack.com/api/oauth.access?client_id=' + req.body.clientId + '&client_secret=' + process.env.SLACK_CLIENT_SECRET + '&code=' + req.body.code + '&redirect_uri=' + process.env.SLACK_REDIRECT_URI, function (error, response, body) {
        if (!error && response.statusCode == 200) {
            res.json(JSON.parse(response.body));
        } else {
            res.status(400).send({
